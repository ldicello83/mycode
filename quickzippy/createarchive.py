...
def zipdir(dirpath, zipfileobj):
    ...
    for root, dirs, files in os.walk(dirpath):
        for file in files:  #we only want to loop across the file component
            print(os.path.join(root,file))  #create an absolute path of where files are
            zipfileobj.write(os.path.join(root, file))  #adds files to our zipfileobject that was passed
    return None  #when we are done, no need to return any value

def main():
    """called at runtime"""
# ask user for directory to be archived
dirpath = input("What directory are we archiving today? ")
## If directory exists, then begin archiving it
if os.path.isdir(dirpath):
    zippedfn = input("What should we call the finished archieve? ")

## zippfn is the zipped file for the archive
    with zipfile.ZipFile(zippedfn, "w", zipfile.ZIP_DEFLATED) as zipfileobj:
## create a zip fle object - we are making a new zip file
        zipdir(dirpath, zipfileobj)  # call our function
else:
    print("Run the cript again when you have a valid directory to zip.")
